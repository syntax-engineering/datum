...
# features include:
#
# 1) The method data_test, for making test cases which are generated from
#    datasets you define.
# 2) Scenarios, which are a load-on-demand mechanism for seeding the test db.
#
# The Datum Parts & API:
#   + Datum.[Class Attributes] are calls to Datum and Scenario paths as
#     well as access to all Containers.
#
#   + Top Level Namespace provides access to data_test (for use in your
#     tests directly) and a couple of basic Scenario helpers
#     (__import, __clone, etc).
#
