...
module Datum
  module Test
    module InstanceMethods
      ...
      def after_setup
        process_scenarios(self.queued_scenarios)
        super
      end

      def queue_scenario(scenario_name)
        self.queued_scenarios = self.queued_scenarios << scenario_name
      end
      ...
      def process_scenarios(scenarios = nil)
        scenarios = self.queued_scenarios if scenarios.blank?
        ...
        self.queued_scenarios = [] if scenarios == self.queued_scenarios
      end
    end
    ...
    module ClassMethods
      ...
      def data_test(name, &block)
        ...
        class_eval(::Datum::Helpers.read_file(name, ::Datum.data_path))
      end
    end
  end
end
